...
class Phoenix_WirecardQPay_Block_Qpaycheckout extends Mage_Core_Block_Template
{
    /**
    ...
    public function getIframeUrl()
    {
        ...
        {
            $redirectUrl = $session->getQPayRedirectUrl();
        }
        else
        {
            try
            {
                $redirectUrl = $this->_getOrder()->getPayment()->getMethodInstance()->getUrl();
            }
            catch(Exception $e)
            {
                $quoteId = $this->_getCheckout()->getLastQuoteId();
                if ($quoteId) {
